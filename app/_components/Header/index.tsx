...
export default function Header({
    ...
    return (
        ...
        >
            ...
            <Navigation
                ...
                firstFocusItem={navOpenFocusRef}
            />
            <EightBitMe
                refObj={eightBitMe}
                onClick={() => setNavOpen(!navOpen)}
                miniMe={size !== HeaderSize.Large}
            />
            <p
                className={`jd-header__scroll-indicator${hasScrolled || scrollReminder ? " jd-header__scroll-indicator--hide" : ""}`}
            >
    ...
    function renderTitle(main?: MainItem) {
        return (
            <h1
                className={`jd-header__title${main ? " jd-header__title--active" : ""}`}
            >
                <div className="jd-header__title-skeleton">
                    <SkeletonText />
                    <SkeletonText />
                </div>
                <Link href="/" className="jd-header__name" data-page={`home`}>
                    {main?.name}
                </Link>
