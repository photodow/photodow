import { AssetType, PosterSizes, TMDB, TMDB_Config, TMDB_Result } from "../../_types/TMDB";
...
function getData (pageCount: number): Promise<TMDB_Result[]> {
    ...
    const storage = JSON.parse(localStore().getItem(key) || 'null') as TMDB_Result[];
    ...
    for (let i = 1; i < pageCount; i++) {
        waitForIt.push(getResultsByType(AssetType.Movie, i).then(d => results.push(d)));
        waitForIt.push(getResultsByType(AssetType.TV, i).then(d => results.push(d)));
    }
...
function getResultsByType (type: AssetType, pageNum: number): Promise<TMDB_Result[]> {
...
function isDataExpired () {
    // refresh in 7 days
    return Number(localStore().getItem(StorageKeys.TMDB_Date)) < new Date().valueOf();
}

function setExpirationDate () {
    // refresh in 7 days
    localStore().setItem(StorageKeys.TMDB_Date, `${new Date(new Date().valueOf() + (1000 * 60 * 60 * 24 * 7)).valueOf()}`);
}

export function createImageURL (baseUrl: string, size: PosterSizes, path: string) {
